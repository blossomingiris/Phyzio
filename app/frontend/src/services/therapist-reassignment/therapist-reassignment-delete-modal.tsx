...
export function TherapistReassignmentDeleteModal({
  therapistId,
  therapistName,
  title,
  confirmLabel,
  ...
  title: string;
  confirmLabel: string;
  noClientsDescription: ReactNode;
  opened: boolean;
  onClose: () => void;
  ...
  const clientsQuery = rqClient.useQuery(
    "get",
    "/clients/",
    { params: { query: { therapistId, limit: 100 } } },
    { enabled: opened },
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      try {
        ...
        // the caller's delete mutation already surfaces a notification on failure.
      }
    } finally {
    ...
    >
      <Stack gap="lg">
        ...
        {!isLoading && !hasAffectedClients && (
          <Text size="md">{noClientsDescription}</Text>
        )}
