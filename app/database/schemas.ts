...
export const originEnum = pgEnum("origin", ["whats_up", "phone", "walk_in", "other"]);

export const communicationEnum = pgEnum("communication", ["whats_up", "phone", "email"]);
...
export const clients = pgTable("clients", {
  ...
  preferredCommunication: communicationEnum("preferred_communication").notNull().default("email"),
...
export const treatmentPlanItems = pgTable(
  ...
  (table) => [
    ...
    check("quantity_completed_non_negative", sql`${table.quantityCompleted} >= 0`),
  ]
);
...
export const appointments = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    therapistId: integer("therapist_id").references(() => therapists.id, { onDelete: "restrict" }),
  ...
  (table) => [
    check("ended_at_after_started_at", sql`${table.endedAt} > ${table.startedAt}`),
  ]
);
